...
if ($argc < 2) {
    printf("usage: $argv[0] station_id\n");
    exit(1);
}
$iStationId = (int) $argv[1];

$sQuery1 = "SELECT t1.localtijd, t1.unixtime, t1.regen, t1.regen7d, t2.localtijd, t2.unixtime, t2.regen, t2.regen7d FROM knmi_th t1, knmi_th t2 WHERE t2.unixtime = t1.unixtime + 3600. AND t1.regen IS NOT NULL AND t2.regen7d IS NULL  AND t1.knmi_station = $iStationId AND t2.knmi_station = $iStationId";
...
foreach ($aaResult1 as $aRow) {
        ...
        $sQuery3 = "UPDATE knmi_th SET regen7d = $fRegen7dNieuw WHERE unixtime = $iUnixtime2 AND knmi_station = $iStationId";
        # printf("$sLocaltijd2   $sQuery3\n");
        # printf("Query3=$sQuery3\n");
        queryw($sQuery3);
}
